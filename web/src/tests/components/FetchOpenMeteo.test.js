...
describe('FetchOpenMeteo', () => {
  ...
  it('defaults to the 2025 full-year range and exposes the casual analyze action', async () => {
    const { getByLabelText, getByRole, queryByLabelText } = render(FetchOpenMeteo);
    ...
    expect(getByLabelText(/API URL/i).value).toContain('start_date=2025-01-01');
    expect(getByLabelText(/API URL/i).value).toContain('end_date=2025-12-31');
    expect(getByLabelText(/Start Date/i)).toHaveValue('2025-01-01');
    expect(getByLabelText(/End Date/i)).toHaveValue('2025-12-31');
  });
